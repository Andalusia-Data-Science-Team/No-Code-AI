...
def missing_data():
    return pd.DataFrame({
        ...
        'City': [np.nan, np.nan, 'Chicago', 'Houston', 'Chicago', 'Houston', 'Houston'],
        'Empty_Col': [np.nan] * 7
...
@pytest.mark.parametrize("input", [
    (pd.DataFrame({'Date': ["2024-01-01", "2024-01-03", "2024-01-05"], 'Target': [25, 30, 30]})),
    (pd.DataFrame({'Date': ["2024-01-01 00:01", "2024-01-01 00:02", "2024-01-01 00:03"], 'Target': [25, 30, 30]})),
    (pd.DataFrame({'Date': ["01/01/2024", "02-01-2024", "3 Jan 2024"], 'Target': [25, 30, 30]})),
    (pd.DataFrame({'Date': ["2024-01-01 00:01:01", None, "2024-01-01 00:03:00"], 'Target': [25, 30, 30]})),
    (pd.DataFrame({'Date': ["2024-01-01", np.nan, "2024-01-05"], 'Target': [25, 30, 30]})),
    (pd.DataFrame({'Date': ["2024-01-01", "invalid", "2024-01-05"], 'Target': [25, 30, 30]})),
...
def test_resample_datetime_conversion(input):
    ...
    assert resampled_df['Date'].dtype == 'datetime64[ns]', "Date Column Wasn't Parsed Correctly"
...
def test_resample_higher_freq(resample_data):  # Case of entered frequency higher than actual data frequency
    expected = pd.DataFrame({
        'Date': pd.to_datetime([
            "2024-01-31 00:00:00"]),
        'Target': [27.5],
...
def test_resample_duplicate_timestamps():
    ...
    df_with_duplicates = pd.DataFrame({
        "date": pd.to_datetime(["2024-01-01", "2024-01-01", "2024-01-03", "2024-01-07"]),
    ...
    assert df_resampled.loc[df_resampled["date"] == "2024-01-01", "target"].iloc[0] == 12.5, "Duplicate handling incorrect"

    # Ensure interpolation filled missing values
    assert df_resampled["target"].isna().sum() == 0, "Missing values were not interpolated"
...
def test_resample_one_day_to_hourly():
    ...
    assert len(df_resampled) == 25, "Resampling did not generate the expected number of timestamps"
    ...
    assert df_resampled["target"].isna().sum() == 0, "Interpolation did not remove all NaNs"
...
def test_descriptive_analysis_mixed_dataframe(analysis_data):
    ...
    num_desc, cat_desc, d_types, missing, dups, unq = descriptive_analysis(analysis_data)
    ...
    assert missing.loc["num_col", "missing %"] == 20.0, "Missing % for num_col incorrect"
    assert missing.loc["cat_col", "missing %"] == 20.0, "Missing % for cat_col incorrect"
